...
old_buildiso=$(grep -oP "arch-bokiVersion='v\d{2}\.\d{2}\.\d{2}'" installation-scripts/build-the-iso.sh | grep -oP 'v\d{2}\.\d{2}\.\d{2}' | head -1)
old_profiledef=$(grep -oP 'arch-boki-v\d{2}\.\d{2}\.\d{2}' archiso/profiledef.sh | grep -oP 'v\d{2}\.\d{2}\.\d{2}' | head -1)
...
# Replace entire arch-bokiVersion='...' line
sed -i "s|\(.*arch-bokiVersion='\)[^']*\('.*\)|\1$newversion\2|" installation-scripts/build-the-iso.sh

# Replace entire iso_label="arch-boki-..." line
sed -i "s|^iso_label=\"arch-boki-.*\"|iso_label=\"arch-boki-$newversion\"|" archiso/profiledef.sh
